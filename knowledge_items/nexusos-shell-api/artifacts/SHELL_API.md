...
REGISTER_SHELL_COMMAND(foo, "[args]", " -o, --option Description", "Short one liner", "Full usage block", "system", cmd_foo);
```
...
2. `args`: String representing dynamic usage syntax (e.g., `"[file]"`). Appears in `Usage: foo [args]`.
3. `options`: Formatted string for options table. Lines must start with space and use ` -o, --opt Description` format.
4. `short_desc`: A highly concise, single-line explanation. This maps exactly into the overarching `help` categorized menu.
5. `usage_help`: A larger string block. This string is displayed automatically when the user types `foo --help`, `foo -h`, or `help foo`. 
6. `category`: Must neatly conform to existing domains (`"system"`, `"fs"`, `"diag"`, `"process"`).
7. `handler`: The C function pointer serving the command.
...
```c
REGISTER_SHELL_COMMAND_EXT(ls, "[dir]", " -l Long format", "List dir", "Display files", "fs", cmd_ls, true);
```
