...
> **Version**: 1.2 | **Last Updated**: 2026-04-17
...
| BUG-006 | `kernel/src/lib/printf.c` — `vsnprintf` | `vsnprintf` handled only `%s/%d/%c/%%`. All hex/unsigned/long specifiers (`%x`, `%X`, `%u`, `%lu`, `%lx`, `%lX`, `%lld`, `%llu`, `%p`) hit `default:` branch — emitted literal `%x` chars, consumed no va_args → stack corruption / UB. `lspci` used `%02x`/`%04x` (→ crash). `free` used `%lu` (→ garbage MiB values). Fix: full `vsnprintf` rewrite with flag/width/length-mod parser and inline `RENDER_UINT` macro. Build: 0 errors 0 warnings all 3 debug levels. | Bug-Fix Agent | 2026-04-17 |
